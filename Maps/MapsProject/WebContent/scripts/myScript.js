...
$(document).ready(function(){
	...
	var set=Set();
	set.addList([1,2,3]);
	set.each(function(elem){alert(elem);});
});
...
var constraints=function(){
	var constraintList=[];
	return {
		//Each contstraint has two targets, and a distance with unit km
		addConstraint:function(constraint){
			constraintList.push(constraint);
		},
		getConstrainedTargetLocations:function(){
			var constrainedTargetSet=Set();
			//For each constraint find all locations that match that constraint and add them to set
			$.each(constraintList,function(index,constraint){
				var target1Locations=targets.getTargetLocations(constraint.target1);
				/*For this constraints target1 find all of its locations.
				 * iterate through ecah of these locations and if it is within distance of
				 * any of target2s locations add both to set */
				$.each(target1Locations,function(index,target1Location){
					var target2LocationsInDistance=getAllTargetLocationsInRadius(constraint.target2,
												target1Location.latlngCenter,
												constraint.distance);
